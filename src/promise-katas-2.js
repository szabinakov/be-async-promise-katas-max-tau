...
const food = () => {
const foodData = fetch("food").then((item) => {return item.data});
return foodData;
};
...
const cat = () => {
  const catList = fetch("cats").then((listOfCats) => {return listOfCats.data.cats.sort()});
  return catList;
};
...
const dog = () => {
  function compareNaughtiness(a, b) {return b.naughty - a.naughty}
  const naughtyPooch = fetch("dogs")
  .then((listOfDogs) => {return listOfDogs.data.dogs})
  .then((dogList) => {return dogList.sort((compareNaughtiness))})
  .then((naughtiest) => {return naughtiest[0]})
  return naughtyPooch;
};
